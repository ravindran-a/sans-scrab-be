...
function createGameState(mode: 'single' | 'ai' | 'multiplayer', aiDifficulty?: number): GameState {
...
function makeMove(
  ...
  if (game.currentTurn % game.players.length !== playerIndex) return { error: 'Not your turn' };

  const player = game.players[playerIndex];

  // Rack index bounds
  for (const idx of rackIndices) {
    if (idx < 0 || idx >= player.rack.length) return { error: 'Invalid rack index' };
  }
  ...
  if (sortedNeeded.length !== sortedRack.length || sortedNeeded.some((c, i) => c !== sortedRack[i])) {
  ...
  return { moveScore: totalScore, wordsFormed: wordScores.map(ws => ws.word) };
}
...
function passTurn(game: GameState, userId: string): { error?: string } {
  ...
  game.currentTurn += 1;
  game.moves.push({ playerId: userId, placements: [], wordsFormed: [], score: 0 });
...
function exchangeTiles(game: GameState, userId: string, rackIndices: number[]): { error?: string } {
  if (game.status !== 'active') return { error: 'Game is not active' };
  const playerIndex = game.players.findIndex(p => p.userId === userId);
  if (playerIndex === -1) return { error: 'Player not in game' };
  if (game.currentTurn % game.players.length !== playerIndex) return { error: 'Not your turn' };
  if (game.tileBag.length < rackIndices.length) return { error: 'Not enough tiles in bag' };
...
describe('Full Game Flow', () => {
  ...
  describe('Solo Mode — Single Player', () => {
    ...
    it('should allow placing a single akshara on center as first move', () => {
      ...
      const result = makeMove(game, 'player1',
        [{ row: CENTER, col: CENTER, akshara }], [0]);
    ...
    it('should play multiple consecutive turns (always player turn)', () => {
      ...
      makeMove(game, 'player1',
        [{ row: CENTER, col: CENTER + 2, akshara: a3 }], [0]);
    ...
    it('should NOT end game on one pass followed by a move', () => {
      ...
      const a1 = constructAkshara([rack[0]], 'अ');
      makeMove(game, 'player1',
        [{ row: CENTER, col: CENTER, akshara: a1 }], [0]);
      expect(game.status).toBe('active');
    ...
    it('should accumulate scores across turns', () => {
      ...
      const a1 = constructAkshara([rack[0]], 'अ');
      const a2 = constructAkshara([rack[1]], 'अ');
      const move1 = makeMove(game, 'player1',
        [
          { row: CENTER, col: CENTER, akshara: a1 },
          { row: CENTER, col: CENTER + 1, akshara: a2 },
        ], [0, 1]);
      ...
      const newRack = game.players[0].rack;
      const a3 = constructAkshara([newRack[0]], 'अ');
      const move2 = makeMove(game, 'player1',
        [{ row: CENTER, col: CENTER + 2, akshara: a3 }], [0]);
    ...
    it('should handle conjunct akshara placement', () => {
      ...
      const result = makeMove(game, 'player1',
        [{ row: CENTER, col: CENTER, akshara }], [0, 1]);
  ...
  describe('AI Mode — Player vs AI', () => {
    ...
    it('should alternate turns between player and AI', () => {
      ...
      const newRack = game.players[0].rack;
      const a3 = constructAkshara([newRack[0]], 'अ');
      const badMove = makeMove(game, 'player1',
        [{ row: CENTER, col: CENTER + 2, akshara: a3 }], [0]);
    ...
    it('should verify AI anti-cheat also passes', () => {
      ...
      makeMove(game, 'player1',
        [{ row: CENTER, col: CENTER, akshara: pa }], [0]);

      // Now AI's turn — verify a correct move works
      const aiRack = game.players[1].rack;
      const aiA = constructAkshara([aiRack[0]], 'अ');
      const result = makeMove(game, 'ai',
        [{ row: CENTER, col: CENTER + 1, akshara: aiA }], [0]);
    ...
    it('should reject AI move with wrong rack indices', () => {
      ...
      const pRack = game.players[0].rack;
      makeMove(game, 'player1',
        [{ row: CENTER, col: CENTER, akshara: constructAkshara([pRack[0]], 'अ') }], [0]);
      ...
      if (aiRack[5] !== aiRack[0]) {
        const result = makeMove(game, 'ai',
          [{ row: CENTER, col: CENTER + 1, akshara: wrongA }], [5]);
  ...
  describe('AI Player Logic', () => {
    ...
    it('should return null on empty board with no dictionary (no valid moves)', async () => {
      ...
      const board = createBoard();
      const rack = ['क', 'म', 'ज', 'ल', 'र', 'त', 'न', 'प', 'स', 'ह', 'ग', 'च', 'द', 'य', 'व'];
  ...
  describe('Multiplayer Mode — Online', () => {
    ...
    it('should enforce turn order: player1 first, then player2', () => {
      ...
      const result = makeMove(game, 'player2',
        [{ row: CENTER, col: CENTER, akshara: a1 }], [0]);
    ...
    it('should alternate turns correctly between two players', () => {
      ...
      const result = makeMove(game, 'player2',
        [{ row: CENTER + 1, col: CENTER, akshara: a3 }], [0]);
      expect(result.error).toBeUndefined();
      expect(game.currentTurn).toBe(2);
    ...
    it('should track scores independently for each player', () => {
      ...
      const p2Rack = game.players[1].rack;
      const a3 = constructAkshara([p2Rack[0]], 'अ');
      const move2 = makeMove(game, 'player2',
        [{ row: CENTER + 1, col: CENTER, akshara: a3 }], [0]);
  ...
  describe('Full Multi-Turn Simulations', () => {
    it('solo: should play 5+ turns building a grid', () => {
      ...
      for (let t = 0; t < 4; t++) {
        ...
        const hasAdjacent =
          ...
          (targetRow < BOARD_SIZE - 1 && game.board[targetRow + 1][CENTER].akshara !== null);
        if (!hasAdjacent) continue;

        res = makeMove(game, 'player1',
          [{ row: targetRow, col: CENTER, akshara }], [0]);
        if (!res.error) turns++;
      }
    ...
    it('multiplayer: should play alternating 6 turns', () => {
      ...
      makeMove(game, 'player2',
        [{ row: CENTER + 1, col: CENTER, akshara: p2a1 }], [0]);
      ...
      for (let t = 2; t < 6; t++) {
        ...
        const hasAdj = targetRow > 0 && game.board[targetRow - 1][CENTER].akshara !== null;
        if (!hasAdj) {
        ...
        const res = makeMove(game, userId,
          [{ row: targetRow, col: CENTER, akshara }], [0]);
    ...
    it('AI mode: should simulate player move then AI pass cycle', () => {
      ...
      const newRack = game.players[0].rack;
      const a3 = constructAkshara([newRack[0]], 'अ');
      const res = makeMove(game, 'player1',
        [{ row: CENTER, col: CENTER + 2, akshara: a3 }], [0]);
    ...
    it('should apply triple letter multiplier', () => {
      ...
      for (let r = 0; r < BOARD_SIZE; r++) {
        for (let c = 0; c < BOARD_SIZE; c++) {
          if (board[r][c].type === 'triple_letter') { tlRow = r; tlCol = c; break; }
        }
        if (tlRow >= 0) break;
      }
  ...
  describe('Anti-Cheat — Deep', () => {
    ...
    it('should handle duplicate consonants in rack', () => {
      const rack = ['क', 'क', 'म', 'म', 'ज', 'ल', 'र', 'त', 'न', 'प', 'स', 'ह', 'ग', 'च', 'द'];
  ...
  describe('Board Premium Square Layout', () => {
    it('should have correct number of each premium type', () => {
      const board = createBoard();
      let tw = 0, dw = 0, tl = 0, dl = 0, center = 0, normal = 0;
      for (let r = 0; r < BOARD_SIZE; r++) {
        for (let c = 0; c < BOARD_SIZE; c++) {
          switch (board[r][c].type) {
            case 'triple_word': tw++; break;
            case 'double_word': dw++; break;
            case 'triple_letter': tl++; break;
            case 'double_letter': dl++; break;
