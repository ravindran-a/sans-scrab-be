...
function generateAccessToken(payload: { userId: string; username: string; subscription: string }): string {
  return jwt.sign(payload, ENV.JWT_SECRET, { expiresIn: ACCESS_TOKEN_EXPIRY });
}

function generateRefreshToken(payload: { userId: string; username: string; subscription: string }): string {
...
describe('Auth & Token System', () => {
  ...
  describe('Token Expiry Configuration', () => {
    ...
    it('should reject an expired refresh token', () => {
      const token = jwt.sign(testUser, ENV.JWT_REFRESH_SECRET, { expiresIn: '-1s' });
  ...
  describe('Auth Middleware Logic', () => {
    ...
    function simulateMiddleware(authHeader: string | undefined): { status?: number; error?: string; userId?: string } {
  ...
  describe('Token Refresh Flow (unit logic)', () => {
    ...
    it('simulated refresh: expired access + valid refresh → new access token', () => {
      ...
      const expiredAccess = jwt.sign(testUser, ENV.JWT_SECRET, { expiresIn: '-1s' });
  ...
  describe('Socket Auth Simulation', () => {
    ...
    function simulateSocketAuth(handshakeAuth: { token?: string }): { error?: string; userId?: string } {
    ...
    it('should accept socket reconnection with refreshed token', () => {
      ...
      const expiredToken = jwt.sign(testUser, ENV.JWT_SECRET, { expiresIn: '-1s' });
