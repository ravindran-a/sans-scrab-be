...
const timerCache = new Map<
  string,
  { roomId: string; turnTimer: number; turnStartedAt: number }
>();
...
function startTurnTimer(
  ...
  const interval = setInterval(() => {
    ...
    const timeLeft = Math.max(
      0,
      cached.turnTimer - Math.floor((Date.now() - cached.turnStartedAt) / 1000),
    );
  ...
  const timeout = setTimeout(async () => {
    ...
    try {
      ...
      const updatedGame = await GameService.passTurn(
        gameId,
      ...
      if (updatedGame.roomId) {
        ...
        io.to(updatedGame.roomId).emit(SOCKET_EVENTS.GAME_TIMER, {
          gameId,
          timeLeft: 0,
          autoPass: true,
...
export function initSocketServer(httpServer: HttpServer): Server {
  ...
  if (redisClient && redisSub) {
    try {
      ...
      console.warn(
        "[Socket] Redis adapter not available, using default adapter",
      );
    }
  } else {
  ...
  io.on(SOCKET_EVENTS.CONNECTION, (rawSocket: Socket) => {
    ...
    socket.on(
      ...
      rateLimited(async (data: { isPrivate?: boolean; turnTimer?: number }) => {
        try {
    ...
    socket.on(
      ...
      rateLimited(async (data: { roomId: string }) => {
        try {
          ...
          if (room.players.length >= room.maxPlayers) {
            ...
            startTurnTimer(
              io,
              joinedGame._id.toString(),
              joinedGame.turnTimer,
              joinedGame.turnStartedAt!,
            );
          }
        } catch (err: any) {
          socket.emit(SOCKET_EVENTS.ERROR, { message: err.message });
    ...
    socket.on(
      ...
      rateLimited(async (data: { roomId: string }) => {
        try {
          ...
          if (room) {
            io.to(data.roomId).emit(SOCKET_EVENTS.ROOM_LEFT, {
              room,
      ...
      rateLimited(async () => {
        try {
          ...
          if (opponent) {
            ...
            await RoomManager.addToMatchmaking(
              socket.userId,
              socket.username,
              user.elo,
            );
    ...
    socket.on(
      SOCKET_EVENTS.GAME_PASS,
      rateLimited(async (data: { gameId: string }) => {
        try {
    ...
    socket.on(
      SOCKET_EVENTS.GAME_EXCHANGE,
      rateLimited(async (data: { gameId: string; rackIndices: number[] }) => {
        try {
          const game = await GameService.exchangeTiles(
            data.gameId,
            socket.userId,
            data.rackIndices,
          );
    ...
    socket.on(
      ...
      rateLimited(async (data: { gameId: string }) => {
        try {
          const game = await GameService.abandonGame(
            data.gameId,
            socket.userId,
          );
          clearTurnTimer(data.gameId);
    ...
    socket.on(
      ...
      rateLimited(async (data: { roomId: string; message: string }) => {
    ...
    socket.on(
      ...
      rateLimited(async (data: { roomId: string; gameId: string }) => {
    ...
    socket.on(
      SOCKET_EVENTS.REMATCH_ACCEPT,
      rateLimited(async (data: { roomId: string; oldGameId: string }) => {
        try {
          const oldRoom = await RoomManager.getRoom(data.roomId);
          if (!oldRoom || oldRoom.players.length < 2) {
            socket.emit(SOCKET_EVENTS.ERROR, {
              message: "Room no longer valid",
            });
            return;
          }
          ...
          if (player) {
            ...
            if (
              ...
              const timeLeft = Math.max(
                ...
                game.turnTimer -
                  Math.floor(
                    (Date.now() - new Date(game.turnStartedAt).getTime()) /
                      1000,
                  ),
              );
              socket.emit(SOCKET_EVENTS.GAME_TIMER, {
                gameId: data.gameId,
                timeLeft,
              });
            }
...
async function handleGameOver(io: Server, game: any) {
  ...
  if (game.mode === "multiplayer" && game.winner && game.winner !== "ai") {
    const loserId = game.players.find(
      (p: any) => p.userId !== game.winner,
    )?.userId;
    if (loserId) {
      try {
        const { winnerChange, loserChange } =
          await LeaderboardService.updateElo(game.winner, loserId);
        game.eloChange = {
          [game.winner]: winnerChange,
          [loserId]: loserChange,
        };
        await game.save();
      } catch (err) {
